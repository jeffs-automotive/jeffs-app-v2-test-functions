...
/**
 * revertKeytagToAssigned — Server Action.
 * Pattern A confirmation when prior_status was posted_ar.
 */
import { z } from "zod";
...
import {
  ...
  type RevertKeytagToAssignedArgs,
  type RevertKeytagResult,
...
export type RevertKeytagState =
  ...
  | {
      kind: "success";
      data: Extract<RevertKeytagResult, { ok: true }>;
    }
  | {
      kind: "tool_error";
      data: Extract<RevertKeytagResult, { ok: false; error_code: string }>;
    }
  | { kind: "transport_error"; message: string };

async function revertKeytagImpl(
  _prev: RevertKeytagState,
  formData: FormData,
): Promise<RevertKeytagState> {
  ...
  try {
    const data = await callKeytagTool("revertKeytagToAssigned", parsed.data, email);
...
export const revertKeytagAction = wrapAdminAction(
  "revertKeytagToAssigned",
  revertKeytagImpl,
  { orchestratorTool: "revertKeytagToAssigned" },
);
