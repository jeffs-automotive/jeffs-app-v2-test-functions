// manual-reviews.ts — browse/search manual reviews for the admin-app list UI.
//
// Ported (Node idiom) from supabase/functions/_shared/tools/manual-review-list.ts
// (listManualReviewsTool + toListItem + reviewMatchesSearch). The ONLY mechanical
// changes from the edge source are the `@supabase/supabase-js` import specifier
// and the extensionless local import of the manual-review types. The two queries
// (the main list select with order+limit, and the head:true open-count), the
// search filter, and the result shaping are IDENTICAL so the direct read matches
// the orchestrator path byte-for-byte.
//
// Read-only. NOT subject to the brute-force lockout that guards
// `lookup_manual_review` — that protects against code-guessing; this is a browse
// over the operator's own shop data behind the admin Entra gate.
