...
/**
 * ViewAttachmentButton (admin-app) — service advisors view the parts-invoice image attached
 * to the Bill/Purchase in QuickBooks, mirroring qteklink-app. Fetches on demand via the admin
 * fetchAttachmentsAction (→ qteklink's internal endpoint). The fetch state lives in a child
 * mounted inside the dialog, so it re-mounts fresh each open (the QBO URL is short-lived). A
 * load skeleton + broken-image fallback keep both async states designed.
 */
...
import { fetchAttachmentsAction } from "@/actions/back-office/fetch-attachments";

function AttachmentImage({ uri, name }: { uri: string; name: string | null }) {
  const [errored, setErrored] = useState(false);
  if (errored) {
    return (
      <a href={uri} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm text-primary underline">
...
function AttachmentContent({ qboTxnType, qboTxnId }: { qboTxnType: string; qboTxnId: string }) {
  ...
  return (
    <>
      ...
      {!pending && state?.ok && state.attachments.length === 0 && (
...
export function ViewAttachmentButton({ qboTxnType, qboTxnId }: { qboTxnType: string; qboTxnId: string }) {
  return (
    <Dialog>
