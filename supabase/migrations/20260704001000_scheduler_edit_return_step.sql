-- =====================================================================
-- edit_return_step column + apply_wizard_transition CASE arm
-- Created 2026-07-04 — summary-edit-hub feature (task EH1)
-- =====================================================================
-- The summary edit hub (new wizard step `summary_edit_hub`) lets the
-- customer edit any one section (contact / vehicle / services / time)
-- from the summary card and return to the hub afterward, with every
-- other section's data preserved (docs/scheduler/summary-edit-hub-plan.md).
--
-- The return-to-hub semantics need a per-session breadcrumb: when a
-- section-edit action (submit-customer-info-edit, submit-vehicle-pick,
-- submit-service-and-concern-picker) sees `edit_return_step =
-- 'summary_edit_hub'`, it routes back to the hub instead of its forced
-- forward chain. The flag stays set across multiple section edits and is
-- cleared explicitly (to SQL NULL) on hub→summary "done", on the
-- date/time flow's natural landing on summary, and by start-over.
--
-- New column: customer_chat_sessions.edit_return_step TEXT (nullable).
--
-- The apply_wizard_transition RPC allowlists columns via CASE-WHEN-? arms;
-- this migration recreates it FROM THE LATEST FULL DEFINITION
-- (20260703080000_scheduler_concern_clarify_column.sql — verified
-- most-recent CREATE OR REPLACE) adding a plain-TEXT arm for
-- edit_return_step (same shape as current_step / channel: `p_payload ?
-- 'edit_return_step'` → p_payload->>'edit_return_step', which yields SQL
-- NULL when the caller passes an explicit JSON null, giving us the
-- clear-the-flag behavior for free via the TEXT ->> operator). All other
-- columns' behavior preserved exactly. SECURITY INVOKER is asserted
-- EXPLICITLY (the 2026-05-25 hardening — Validator 1 H1).
-- =====================================================================
...
ALTER TABLE public.customer_chat_sessions
  ADD COLUMN edit_return_step TEXT;

COMMENT ON COLUMN public.customer_chat_sessions.edit_return_step IS
  'Summary edit hub (2026-07-04): breadcrumb set to ''summary_edit_hub'' while the customer is editing a single section (contact/vehicle/services/time) reached from the hub. Section-edit actions read it to route BACK to the hub instead of their forced forward chain; it survives multiple section edits and is cleared (to NULL) on hub→summary "done", on the date/time flow reaching summary, and by start-over. NULL means the customer is on the normal forward flow.';

-- ────────────────────────────────────────────────────────────────────
-- 2 / 2 — apply_wizard_transition gains the edit_return_step CASE arm.
-- Plain-TEXT pattern (same as current_step / channel): explicit JSON null
-- clears to SQL NULL via ->>. Full body reproduced via CREATE OR REPLACE
-- from the 20260703080000 definition; all other columns' behavior
-- preserved exactly.
-- ────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  UPDATE public.customer_chat_sessions SET
    ...
    current_step = CASE
      ...
      ELSE current_step
    END,
    -- ─── SUMMARY EDIT HUB (task EH1, 2026-07-04) — new branch ──────────
    -- edit_return_step: plain-TEXT breadcrumb. Same shape as current_step
    -- — an explicit JSON null in the payload yields SQL NULL via ->>,
    -- which is exactly the "clear the flag" behavior the hub-exit /
    -- date-time-to-summary / start-over paths rely on.
