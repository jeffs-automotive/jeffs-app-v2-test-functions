...
// ── Orphan-release email (sent at end of run when orphans were released) ────

const RESEND_API_KEY = Deno.env.get("RESEND_API_KEY") ?? "";
const ORPHAN_TO_EMAIL =
  Deno.env.get("KEYTAG_REPORT_TO_EMAIL") ?? "service@jeffsautomotive.com";
const ORPHAN_FROM_EMAIL =
  Deno.env.get("KEYTAG_REPORT_FROM_EMAIL") ??
  "Jeff's Automotive Key Tags <alerts@jeffsautomotive.com>";

function buildOrphanEmailHtml(orphans: OrphanReleaseDetail[]): string {
  const rows = orphans
    .map(
      (o) => `<tr>
      <td style="padding:8px;border-bottom:1px solid #333;font-family:monospace;font-weight:600;color:#f0a8a8;">${o.tag_color === "red" ? "R" : "Y"}${o.tag_number}</td>
      <td style="padding:8px;border-bottom:1px solid #333;color:#e0e0e0;">RO ${o.ro_number}</td>
      <td style="padding:8px;border-bottom:1px solid #333;color:#bbb;">${o.prior_status === "posted_ar" ? "A/R" : "WIP"}</td>
      <td style="padding:8px;border-bottom:1px solid #333;color:#bbb;font-size:12px;">${o.tekmetric_status_at_release}</td>
      <td style="padding:8px;border-bottom:1px solid #333;color:#999;font-size:12px;">${o.reason}</td>
    </tr>`,
    )
    .join("");
  return `<!DOCTYPE html>
...
<div style="max-width:760px;margin:0 auto;padding:24px;background:#262626;">
<h1 style="margin:0;color:#96003C;font-size:22px;border-bottom:2px solid #D2B487;padding-bottom:8px;">Key Tags Released by Nightly Reconcile</h1>
<p style="margin:12px 0 16px 0;color:#ddd;font-size:14px;">The nightly reconcile cron released ${orphans.length} tag${orphans.length === 1 ? "" : "s"} because the corresponding Tekmetric repair order is no longer reachable or has moved past POSTED_PAID. Please verify each release was correct.</p>
<p style="margin:0 0 12px 0;color:#999;font-size:13px;"><strong>If a release was a mistake:</strong> use Claude Desktop's "Assign Key Tag" tool to re-assign the original color/number to the same RO. The system retains a record of the prior assignment for log purposes only — Tekmetric's keytag field on the dead RO was NOT cleared.</p>
<table role="presentation" style="width:100%;border-collapse:collapse;font-size:13px;margin-top:16px;">
<thead><tr>
  <th style="text-align:left;padding:8px;border-bottom:1px solid #D2B487;color:#ddd;">Tag</th>
  <th style="text-align:left;padding:8px;border-bottom:1px solid #D2B487;color:#ddd;">RO #</th>
  <th style="text-align:left;padding:8px;border-bottom:1px solid #D2B487;color:#ddd;">Prior Status</th>
  <th style="text-align:left;padding:8px;border-bottom:1px solid #D2B487;color:#ddd;">Tekmetric State</th>
  <th style="text-align:left;padding:8px;border-bottom:1px solid #D2B487;color:#ddd;">Reason</th>
</tr></thead>
<tbody>${rows}</tbody>
</table>
<p style="margin:24px 0 0 0;font-size:11px;color:#777;">Sent by keytag-bulk-reconcile. Reply-to is not monitored. Use the Claude Desktop key-tag tools to make changes.</p>
</div></body></html>`;
}

/**
 * Builds an idempotency key fingerprinted on the EXACT orphan set, not just the
 * count. Two different orphan releases with the same count on the same day
 * would have shared a key under a count-based scheme — and Resend's 24h dedup
 * window would have swallowed the second email, hiding releases from the
 * service team. Using a content hash means: same orphan set → dedup; different
 * orphan set → fresh email.
 */
async function orphanIdempotencyKey(
  orphans: OrphanReleaseDetail[],
): Promise<string> {
  const sortedIds = orphans
    .map((o) => `${o.ro_id}:${o.tag_color}${o.tag_number}:${o.reason}`)
    .sort()
    .join("|");
  ...
  const hex = Array.from(new Uint8Array(buf))
    ...
    .join("")
    .slice(0, 16); // 64 bits — collision-resistant enough for daily-volume dedup
  // Resend caps Idempotency-Key length at 256 chars; this is well under.
  return `keytag-orphan-release:${new Date().toISOString().slice(0, 10)}:${hex}`;
}

async function sendOrphanEmail(
  orphans: OrphanReleaseDetail[],
  ...
  if (orphans.length === 0) {
    return { sent: false, error: "no orphans to email" };
  }
  const subject = `${orphans.length} key tag${orphans.length === 1 ? "" : "s"} released by nightly reconcile — please verify`;
  const html = buildOrphanEmailHtml(orphans);
  const idempotencyKey = await orphanIdempotencyKey(orphans);
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      headers: {
        ...
        // Idempotency-Key fingerprints the orphan SET (sorted ro_id + tag + reason).
        // Same set on the same day = dedup; different set = fresh email.
        "Idempotency-Key": idempotencyKey,
      },
      body: JSON.stringify({
        from: ORPHAN_FROM_EMAIL,
        to: [ORPHAN_TO_EMAIL],
        subject,
        html,
      }),
    ...
    if (res.status === 409) {
      // Resend dedup hit — already sent today with this exact orphan set.
      // Treat as success; nothing to do.
      return { sent: true };
    }
    if (!res.ok) {
      const text = await res.text();
      return { sent: false, error: `HTTP ${res.status}: ${text.slice(0, 300)}` };
    }
    return { sent: true };
  } catch (e) {
