...
//   - "Repair Orders Without Key Tags" (unresolved ARN reviews)
//   - Full 180-tag grid (R1..R90, Y1..Y90) — available green, in-use shaded red
//
// Triggered by pg_cron (jobname: keytag-daily-report) at 0 11 * * 1-6 UTC
// = 7 AM EDT, Mon-Sat. Auth: Pattern A bearer check.
//
// 2026-06-18: the DATA build (counts / stale + customer names / ARN rows /
// pool grid) moved to `_shared/keytag-dashboard-data.ts` so the admin-app
// `/keytags` Dashboard tab renders the SAME snapshot the email does. This file
// now only owns the HTML rendering + the Resend send. The email output is
// byte-for-byte unchanged by that refactor.
//
// Email surface:
//   From: Jeff's Automotive Key Tags <alerts@jeffsautomotive.com>
//   To:   service@jeffsautomotive.com
//   Idempotency-Key: keytag-daily-report:YYYY-MM-DD
//
// Env vars:
...
} from "../_shared/keytag-dashboard-data.ts";
...
function buildReportHtml(args: KeytagDashboardData): string {
  const {
    tags,
    inUseCount,
    availableCount,
  ...
  const reds = tags
    .filter((t: KeytagRow) => t.tag_color === "red")
    .sort((a: KeytagRow, b: KeytagRow) => a.tag_number - b.tag_number);
  const yellows = tags
    .filter((t: KeytagRow) => t.tag_color === "yellow")
    .sort((a: KeytagRow, b: KeytagRow) => a.tag_number - b.tag_number);
  ...
  // is clipped in narrower email-client viewports.
  const GRID_COLS = 15;
  ...
  // "Repair Orders Without Key Tags" section. Consolidation surface for ARN
  // (`ar_no_prior_tag`) reviews. Always show the header so the team sees a
  // positive "no issues" signal on quiet days.
...
Deno.serve((req) => withSentryScope(req, "keytag-daily-report", async () => {
  ...
  // `?force=true` bypasses Resend's idempotency-key dedup for ad-hoc smoke
  // tests. Daily cron NEVER passes this.
  const url = new URL(req.url);
  const force = url.searchParams.get("force") === "true";

  // Build the pool snapshot (shared with the admin-app Dashboard tab).
  let data: KeytagDashboardData;
  try {
    data = await buildKeytagDashboardData(sb, SHOP_ID);
  } catch (e) {
    return jsonResponse(
  ...
  const noTagCount = data.rosWithoutKeytags.length;
  const subject =
    noTagCount > 0
      ? `Key Tags: ${data.inUseCount} in use, ${data.availableCount} available, ${data.staleCount} stale, ${noTagCount} A/R without tag`
      : `Key Tags: ${data.inUseCount} in use, ${data.availableCount} available, ${data.staleCount} stale`;
