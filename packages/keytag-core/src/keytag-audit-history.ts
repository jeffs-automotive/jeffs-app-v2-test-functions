// keytag-audit-history.ts — the getKeytagAuditHistory pure-DB read.
//
// COPY for the @jeffs/keytag-core read package (Phase 0 build-seam spike,
// 2026-06-26). Carries ONLY the `getKeytagAuditHistory` read fn + its types out
// of the Deno `supabase/functions/_shared/tools/keytag-extras.ts` (~line 694).
// The rest of `keytag-extras.ts` is Deno-coupled (Tekmetric HTTP client,
// confirmation tokens, Deno-env email leaf) and is NOT carried here.
//
// Seam edits vs source:
//   1. SupabaseClient import → bare specifier `@supabase/supabase-js`.
//   2. `TagColor` is imported from the local `./keytag-format.ts` copy.
// Pure `.from('keytag_audit_log').select()` read — no Tekmetric, no RPC.

import type { SupabaseClient } from "@supabase/supabase-js";
import type { TagColor } from "./keytag-format.ts";
...
export async function getKeytagAuditHistory(
  ...
  if (args.tag_number !== undefined)
    query = query.eq("tag_number", args.tag_number);
  if (args.ro_number !== undefined)
    query = query.eq("ro_number", args.ro_number);
