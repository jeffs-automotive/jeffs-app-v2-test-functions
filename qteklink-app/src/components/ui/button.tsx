 ...
 * forms (ported verbatim from admin-app/src/components/ui/button.tsx so the
 * two internal apps behave byte-for-byte the same):
 *
 ...
 *
 * KNOWN CONTRACT: the child-replacement heuristic assumes button structure
 * of `<Icon /> + "text"`. If your label is wrapped in an element (e.g.,
 * `<span>Save</span>`), it will be treated as the "icon" and replaced by
 * the spinner — text disappears. For non-standard layouts, render the
 * spinner manually instead of using the `loading` prop, or restructure to
 * put a leading icon first.
 */
