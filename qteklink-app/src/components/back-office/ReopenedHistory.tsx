/**
 * ReopenedHistory — the posting-lifecycle timeline for a reopened RO, read from
 * context.history (built by the back-office-ro-watch detector). Each entry carries a
 * pre-formatted shop-local timestamp (at_local), so this is presentation-only. Ascending
 * (first issue first). A connected vertical timeline: each event is a node on a rail, with
 * reopens (unposts) accented so the "posted two days previous" story reads at a glance.
 */
...
function detail(h: HistoryItem): string | null {
  if (h.kind === "ro_posted" || h.kind === "ro_sent_to_ar") {
    ...
    if (typeof h.total_cents === "number") parts.push(centsToUsd(h.total_cents));
