/**
 * Back-office status vocabulary — the shared issue-status + change-type + kind badges.
 * Color + icon (never color alone); tints clear WCAG AA in light AND dark. This file's
 * rendered output is mirrored byte-for-byte in admin-app (only the `IssueStatus` import
 * path differs) so a status reads identically across both apps — the cross-app design
 * contract. Purely presentational; icons are aria-hidden and the visible label is the
 * accessible name (so role/text test queries keep working).
 */
import {
  AlarmClock,
  CalendarClock,
