...
export interface RecordManualPaymentInput {
  ...
  /** Gross paid, integer cents (the RO's amountPaid). */
  amountCents: number;
  /** User-entered CC fee, integer cents (card). */
  ccFeeCents?: number;
  /** ISO timestamp — the paid date. */
  paymentDate: string;
}
 ...
 * Fails closed when the shop has no connection. Throws on DB error. Returns the id.
 */
export async function recordManualPayment(
  ...
  const { data, error } = await admin.rpc("qteklink_record_manual_payment", {
    ...
    p_cc_fee_cents: input.ccFeeCents ?? 0,
