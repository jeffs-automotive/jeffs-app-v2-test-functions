/**
 * Sentry server-side init for qteklink-app (Node runtime).
 *
 * Same PII scrubber shape as admin-app: QTekLink Server Actions surface
 * customer + financial data from QBO / Tekmetric (customer names, RO numbers,
 * amounts) and we must never leak that to Sentry.
 *
...
Sentry.init({
  ...
  initialScope: {
    tags: {
      // shop_id is NOT hardcoded — QTekLink is multi-realm-capable; the shop is
      // resolved per-request (requireQtekUser / wrapQtekAction set the tag).
      surface: "qteklink-app",
    },
  },
...
function scrubEvent(event: Sentry.ErrorEvent): Sentry.ErrorEvent | null {
  // The actor email (from requireQtekUser) IS intentionally captured for
