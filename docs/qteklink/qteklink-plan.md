...
- **Method unknown — user picks (added 2026-06-07):** the RO snapshot has `amountPaid` (that it was paid) but **not how** — the method lives only on the `payment_made` event. When a payment webhook is missing (e.g. a Tekmetric ingestion outage), the daily approval surfaces the paid RO and the **user picks the method** (card / cash / check / non-cash), which routes the payment JE; the amount comes from `amountPaid`. For a manually-classified **card**, the `applicationFee` (CC fee) is also unknown → the **user enters it** (Chris, 2026-06-07). ≈1% of paid ROs (live: 5 of 509). **C6 ships the builder + storage** (`qteklink_manual_payments` table + `buildShopManualPaymentJe` — pure builder is source-agnostic over the projection AND a manual pick). The **C7/C8 daily-approvals UI + poster MUST** (deferred from C6, Chris 2026-06-07): (a) **server-derive the amount** from the RO's `amountPaid` — never trust a client value; and (b) **anti-join** manual picks against `qteklink_payment_state` so a pick can't double-post beside a real (or late-arriving) payment.
