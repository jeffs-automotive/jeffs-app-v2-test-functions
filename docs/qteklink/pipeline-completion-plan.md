> **SUPERSEDED 2026-06-12 (audit):** The D-projection "nightly only" framing below is stale. The day-category projection now **refreshes on every day view** — opening `/approvals` or a day breakdown runs the live-on-view preamble (`reconcileDayForView`: re-reconcile the viewed day unless it's terminal), so a webhook that landed a minute ago is already reflected. The nightly cron (`app/api/cron/daily-sync`) stays as the verification net, not the only refresh. **UPDATE 2026-06-24:** the nightly cron also AUTO-POSTS posted-day CORRECTIONS (`sweepPostedDays`→`applyDayCorrections`, "system (auto-correction)") regardless of `auto_post` — not just verifies; `auto_post` gates first-time posting only. Caveat: once a day's Undeposited is deposited in QBO, its payments/fees JEs are deposit-LOCKED (QBO 6540 → `deposit_locked`); the sales JE never locks. See `docs/qteklink/payments-fixes-findings.md` "## SHIPPED — final state".
