...
- **Filesystem MCP** — `read_file(path)`. Read the subcategory-service-map
  template at this absolute path:

  `C:\Users\ChristopherGoodson\Apps\jeffs-app-v2-test-data\docs\chat-instructions\scheduler\templates\subcategory-service-map.md`
...
- **Orchestrator MCP** — `run_orchestrator(intent, params)`. Pass a clear
  ...
  tool (`upload_subcategory_service_map_md`, `export_subcategory_service_map_md`,
  `revert_md_upload`). See "Two-step flow" below for the exact intent phrasing.
