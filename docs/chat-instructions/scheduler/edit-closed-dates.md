...
```
Step 1: upload_closed_dates_md({ dry_run: true })
        → returns diff + confirm_token

Step 2: Render diff. Wait for "yes".

Step 3: upload_closed_dates_md({
          dry_run: false,
...
scheduler/templates/closed-dates.md` from main automatically. Make sure
advisor pushed edits to main first.
