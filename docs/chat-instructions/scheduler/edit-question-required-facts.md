...
```
Step 1: upload_question_required_facts_md({ dry_run: true })
...
Step 3: upload_question_required_facts_md({
          dry_run: false,
...
scheduler/templates/question-required-facts.md` from the project's main
branch automatically. Source of truth is what's on main — so make sure
the advisor pushed their edits to main BEFORE calling this tool.

**Power-user escape hatches** (rarely needed):

- `source_branch: "feature-x"` — fetch from a feature branch
- `md_content: "..."` — pass inline content directly (legacy path)
