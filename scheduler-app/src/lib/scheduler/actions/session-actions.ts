...
// `appointment_type`. Applies the full 5-layer availability stack per
// chat-design.md §9 + the schema migration spec:
//
...
//      (blocked_type IS NULL OR blocked_type = args.appointment_type)
...
//        - waiter: capacity = waiter_8am_slots + waiter_9am_slots
//                  (after subtracting time-blocked slots). Date is available
//                  if EITHER 8 AM or 9 AM has room.
//        - dropoff: capacity = dropoff_total. Date is available if the
//          combined count is below dropoff_total.
//
// Times for waiter slots are extracted from appointment.start_time using the
// shop timezone (America/New_York) to handle DST correctly. Holds are stored
// with HH:MM in scheduled_time already.
async function computeAvailableDates(args: {
  ...
  // Shop-timezone-aware date+hour extraction. Handles DST since
  // Intl.DateTimeFormat knows the zone's rules.
  function shopLocalDateAndHour(
    ...
    const mm = get("month");
    const dd = get("day");
    // hour="2-digit" + hour12=false returns "00".."23"
    const hh = parseInt(get("hour"), 10);
    return { date: `${yyyy}-${mm}-${dd}`, hour: hh };
  }

  // Bucket counts per date — for waiter we also bucket per slot.
...
export async function dismissEscalation(args: {
  ...
  try {
    ...
    // Order by occurred_at — the timestamp column on scheduler_audit_log per
    // migration 20260513000100. There is no created_at column.
